...
# === NEW/UPDATED: 重写热力图函数，生成“热力图 + 每列均值 + 每行均值”的合成图，并保存到热力图路径 ===
def create_heatmap_with_marginals(
    results,
    save_path=None,                          # 这里传的是文件路径（如 .../heatmap.png）
    ...
    # row 按 5% -> 95%（上到下）排序：barh 默认上到下按索引顺序，所以用升序
    depth_order_idx = np.argsort(depths)  # depths 已经升序，显式写清楚
    row_avgs = pd.Series(np.nanmean(score_matrix, axis=1)[depth_order_idx],
                         index=[row_labels[k] for k in depth_order_idx])
...
def main():
    ...
    if 'heatmap' in args.plot_types:
        # === UPDATED: 用新的函数生成“热力图 + 行/列均值合成图”，保存到 heatmap.png 同一路径 ===
    ...
    print(f"  - heatmap.png: Performance heatmap + per-column & per-row averages (stacked)")
