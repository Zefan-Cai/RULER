...
def _lcs_len(a: str, b: str) -> int:
    """长度为 O(len(a)*len(b)) 的标准 LCS 动态规划"""
    n, m = len(a), len(b)
    dp = [0] * (m + 1)
...
def calculate_score(outputs_gold, output_pred, partial_credit=False, partial_mode: str = "lcs"):
    """
    Calculate score with optional partial credit.

    Args:
        outputs_gold: List[str] gold answers
        output_pred: str model prediction
        partial_credit: bool enable partial credit
        partial_mode: "set" | "lcs" | "token_f1"
            - "set": 原有逻辑（每个 gold 是否为 pred 的子串）
            - "lcs": LCS(gold, pred) / len(gold)，多个 gold 取 max
            - "token_f1": token 级 F1（多个 gold 取 max）
    Returns:
        float in [0,1]
    """
    if not outputs_gold:
    ...
    golds = [str(ans).strip() for ans in outputs_gold]
    pred = str(output_pred).strip()
    golds_lc = [g.lower() for g in golds]
    pred_lc = pred.lower()

    if not partial_credit:
        # 全对：所有 gold 都得在 pred 中出现
        return 1.0 if all(g in pred_lc for g in golds_lc) else 0.0

    # 部分分
    if partial_mode == "set":
        found = sum(1 for g in golds_lc if g in pred_lc)
        return found / len(golds_lc)

    elif partial_mode == "lcs":
        # 对每个 gold 计算归一化 LCS，取 max
        best = 0.0
        for g in golds_lc:
            if len(g) == 0:
                continue
            l = _lcs_len(g, pred_lc)
            best = max(best, l / len(g))
        return best

    elif partial_mode == "token_f1":
        best = 0.0
        for g in golds:
            best = max(best, _token_f1(g, pred))
        return best
...
def process_results(data_file, output_file, partial_credit=True, partial_mode="lcs"):
    ...
    for item in data:
        ...
        score = calculate_score(item['outputs'], model_output, partial_credit, partial_mode)
...
def main():
    ...
    parser.add_argument('--partial_mode', type=str, default='lcs',
                        choices=['set', 'lcs', 'token_f1'],
                        help='Partial credit mode: set | lcs | token_f1')
    ...
    # )
    results, detailed_results = process_results(
        args.data_file, args.output_file, args.partial_credit, args.partial_mode
    )
